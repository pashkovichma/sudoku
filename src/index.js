module.exports = function solveSudoku(matrix) {
  ...
  let count;
  let count1;
  //first iteration filler indexes
  for (i = 0; i < 9; i++){
      for (j = 0; j < 9; j++){
          ...
          for (i = 0; i < 9; i++){
              for (j = 0; j < 9; j++){      
                  ...
                  if (matrix[i][j].length){
                      ...
                      //row
                      for (jj = 0; jj < 9; jj ++){
                          console.log(i,jj,matrix[i][jj]);
                          if (matrix[i][jj].length == matrix[i][j].length && ii != i && matrix[i][jj]){
                              console.log("count", i, jj, 
                              matrix[i][jj].filter(x => matrix[i][j].indexOf(x) == -1),
                              matrix[i][jj].filter(x => matrix[i][j].indexOf(x) == -1).length);
                          }
                         if (matrix[i][jj].length == matrix[i][j].length 
                             && matrix[i][jj].filter(x => matrix[i][j].indexOf(x) == -1).length == 0){
                              count ++;
                          }
                      }
                      if (count  == matrix[i][j].length){
                          for (jj = 0; jj < 9; jj ++){
                              if (j != jj && matrix[i][jj].length && matrix[i][jj].filter(x => matrix[i][j].indexOf(x) == -1).length != 0){
                                  matrix[i][jj]= matrix[i][jj].filter(x=>matrix[i][j].indexOf(x) == -1);
                                  if (matrix[i][jj].length == 1){
                                      matrix[i][jj] = matrix[i][jj][0];
                                  }
                                  flag = 0;
                                  flag1 = 0;
                              }
